...
namespace Mox.UI.Game
{
    ...
    public class StackFanPanel : Panel
    {
        ...
        public static readonly DependencyProperty MouseOverOffsetRatioProperty = DependencyProperty.Register("MouseOverOffsetRatio", typeof(double), typeof(StackFanPanel), new FrameworkPropertyMetadata(0.95, FrameworkPropertyMetadataOptions.AffectsArrange));

        public double MouseOverOffsetRatio
        {
            get { return (double) GetValue(MouseOverOffsetRatioProperty); }
            set { SetValue(MouseOverOffsetRatioProperty, value); }
        }
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            if (Children.Count > 0)
            {
                var offsetRatio = Math.Max(NormalOffsetRatio, MouseOverOffsetRatio);

                width -= foldWidth;
                width += offsetRatio * childWidth;
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            for (int i = 0; i < Children.Count; i++)
            {
                if (i == selectedIndex)
                {
                    var offsetRatio = SelectedIndex < 0 ? NormalOffsetRatio : MouseOverOffsetRatio;

                    x -= foldWidth;
                    x += offsetRatio * childWidth;
                }
