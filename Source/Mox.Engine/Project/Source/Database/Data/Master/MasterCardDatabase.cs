...
namespace Mox.Database
{
    ...
    public class MasterCardDatabase : CardDatabase
    {
        ...
        private MasterCardDatabase()
        {
            ...
            using (Profile("Total"))
            {
                using (Stream setsStream = new MemoryStream(Encoding.UTF8.GetBytes(Resources.setinfo)))
                using (Stream cardsStream = new MemoryStream(Encoding.UTF8.GetBytes(Resources.cards)))
                using (Stream cardInstancesStream = new MemoryStream(Encoding.UTF8.GetBytes(Resources.meta)))
                {
                    parser.Parse(setsStream, cardsStream, cardInstancesStream);
                }
            }
        }
