...
namespace Mox.UI
{
    ...
    public class InteractionControllerMulliganTests : InteractionControllerTester
    {
        ...
        {
            IInteraction<bool> interaction = InteractionController.BeginMulligan();
            {
                ...
                Model.Interaction.SelectChoiceCommand.Execute(yesChoice);
            }
            Assert.IsTrue(interaction.IsCompleted);
            Assert.IsTrue(interaction.Result);
        }
        ...
        public void Test_BeginMulligan_returns_false_if_selecting_the_no_choice()
        {
            IInteraction<bool> interaction = InteractionController.BeginMulligan();
            {
                ...
                Model.Interaction.SelectChoiceCommand.Execute(noChoice);
            }
            Assert.IsTrue(interaction.IsCompleted);
            Assert.IsFalse(interaction.Result);
        }
