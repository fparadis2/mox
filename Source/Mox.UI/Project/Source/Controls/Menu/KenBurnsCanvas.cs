...
namespace Mox.UI
{
    public abstract class KenBurnsCanvas : Canvas
    {
        ...
        private readonly Timer m_startCheckTimer = new Timer();
        private Timer m_contentChangeTimer;
        ...
        private void StartNextStoryboard()
        {
            ...
            m_contentChangeTimer = new Timer { Interval = TimeBetweenContentChange.TotalMilliseconds };
            m_contentChangeTimer.Elapsed += contentChangeTimerElapsed;
        ...
        private void EnableTimers(bool enabled)
        {
            var contentChangeTimer = m_contentChangeTimer;
            if (contentChangeTimer != null)
            {
                contentChangeTimer.Enabled = enabled;
            }
        }
        ...
        void m_startCheckTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
        ...
        void KenBurnsCanvas_Loaded(object sender, RoutedEventArgs e)
        {
            if (!StartFirstStoryboardIfReady())
            {
                m_startCheckTimer.Interval = 200;
                m_startCheckTimer.Elapsed += m_startCheckTimer_Elapsed;
                m_startCheckTimer.Start();
            }
        }

        void contentChangeTimerElapsed(object sender, ElapsedEventArgs e)
        {
            var contentChangeTimer = m_contentChangeTimer;
            if (contentChangeTimer != null)
            {
                contentChangeTimer.Elapsed -= contentChangeTimerElapsed;
